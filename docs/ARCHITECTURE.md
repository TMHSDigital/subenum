...
2.  **Wildcard Detection**: Resolves two random subdomains, keeps the probe answers as a fingerprint, and exits unless `-force` is set. With `-force`, results matching the fingerprint are filtered. Recursive scans also probe each new parent before expanding children.
...
*   **Implementation**:
    ...
    *   Function: `dns.ResolveDomain(ctx, resolver, domain, timeout, verbose) bool` - convenience wrapper returning a boolean.
    ...
    *   Function: `dns.CheckWildcard(ctx, resolver, domain, timeout) (bool, []Record, error)` - resolves two random subdomains to detect wildcard DNS. The union of both probe answers is returned as a fingerprint. In `scan.Run` that fingerprint is stored and any later result whose record set is a subset of it is dropped and counted as `wildcardFiltered`. In recursive mode, `CheckWildcard` also runs against each newly resolved parent before children are enqueued; a wildcard branch is not expanded (cached per parent, one probe pair per branch).
...
*   **Implementation**: The worker pool logic lives in `internal/scan/runner.go` as `scan.Run(ctx, cfg, events)`. Both the CLI (`run()` in `main.go`) and the TUI (`internal/tui`) call this function.
    ...
    *   **`scan.Event` / `scan.EventKind`**: Typed events emitted on a `chan<- scan.Event` - `EventResult`, `EventProgress`, `EventWildcard`, `EventError`, `EventDone`. `EventDone` carries a `Stats` snapshot with per-outcome counters (found, nxdomain, timeout, refused, other).
    *   **Dispatcher and work queue**: A dispatcher goroutine owns an internal `jobs` channel, the queue of pending `job{domain, depth}` items (head index plus periodic compaction so the backing array is not pinned at peak size), a visited set, and a pending-work counter. `-max-queries` stops admission once the cap is reached and emits an informational event with the skip count. When `-recursive` is set, `scan.Run` computes the theoretical ceiling (`sum n^d`) and refuses to start above 1e7 unless `-max-queries` or `-force` is set. It seeds the queue from the wordlist slice and feeds workers.
...
1.  **Input**: The user provides command-line arguments: the target domain, the path to a wordlist file (`-w`), a concurrency level (`-t`), a DNS timeout (`-timeout`), a DNS server (`-dns-server`), attempts (`-attempts`), output options (`-o`, `-format`), and scan tuning (`-rate`, `-type`, `-recursive`, `-depth`, `-max-queries`, `-no-abort`), plus flags for verbose mode (`-v`), progress reporting (`-progress`), and force mode (`-force`). The TUI (`-tui`) gathers the equivalent values from its form instead.
...
3.  **Preflight and wildcard detection**: Inside `scan.Run` the apex is resolved once; any outcome other than Found or NXDOMAIN emits `EventError` naming the resolver and returns. Then two random subdomains are resolved (skipped in simulation mode). The union of those answers is the wildcard fingerprint. If either resolves, an `EventWildcard` is emitted; the scan aborts unless `-force` is set. With `-force`, later results whose records are a subset of the fingerprint are dropped. In recursive mode the same probe is run against each newly resolved parent before expansion; a wildcard branch is skipped. `-recursive` also computes the job ceiling and refuses to start above 1e7 unless `-max-queries` or `-force` is set.
...
7.  **Result Emission**: On a successful resolution the worker increments the found counter and emits an `EventResult` carrying the domain and its typed records. Every attempt, success or failure, increments the matching `Stats` counter. The caller (CLI `Writer` or TUI scan view) renders results; the `Writer` routes results to stdout and any `-o` file in the selected `-format`. After `EventDone` the CLI prints the outcome breakdown to stderr and the TUI summary line shows the same counters.
...
*   **Lookup Failure**: When a DNS lookup fails, the error is classified (`NXDomain`, `Timeout`, `Refused`, `Other`) and counted. NXDOMAIN is a definitive negative and is not treated as an infrastructure failure. Timeouts, refusals, and other errors feed the reliability guard. The CLI always prints the per-outcome breakdown after `EventDone`.
