...
3.  **Wildcard Detection**: Inside `scan.Run`, two random subdomains are resolved against the target domain (skipped in simulation mode). The union of those answers is the wildcard fingerprint. If either resolves, wildcard DNS is detected and an `EventWildcard` is emitted; the scan aborts with an `EventError` unless `-force` is set. With `-force`, later results whose records are a subset of the fingerprint are dropped. In recursive mode the same probe is run against each newly resolved parent before expansion; a wildcard branch is skipped.
